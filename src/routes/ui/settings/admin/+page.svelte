<script lang="ts">
	...
	const tabFromUrl = $page.url.searchParams.get('tab') || 'users';
	...
	async function handleDownloadDb() {
		requestConfirmation(
			'Download Database',
			'Are you sure you want to download the database? This may contain sensitive information.',
			async () => {
				isDownloading = true;
				try {
					const response = await fetch('/api/settings/admin/db');
					if (!response.ok) {
					...
					a.href = url;
					a.download = `molos-database-${new Date().toISOString().split('T')[0]}.db`;
