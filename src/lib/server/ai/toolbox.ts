...
import type { ToolDefinition as AgentToolDefinition } from '@molos/agent';
...
export class AiToolbox {
	...
	/**
	 * Get tools in @molos/agent format for use with AgentLoop.
	 *
	 * This method returns tools without the `execute` function - the execute function
	 * is provided separately via `AgentLoop.setToolExecutor()`.
	 *
	 * @param userId - The user ID
	 * @param activeModuleIds - Optional list of module IDs to load tools from
	 * @param mentionedModuleIds - Optional list of mentioned module IDs (prioritized)
	 * @returns Tools in @molos/agent ToolDefinition format
	 */
	async getToolsForAgent(
		...
		// Get tools in our internal format first
		const tools = await this.getTools(userId, activeModuleIds, mentionedModuleIds);

		// Convert to @molos/agent ToolDefinition format (without execute function)
		return tools.map((tool) => ({
			...
			parameters: {
				...
				properties: (tool.parameters?.properties || {}) as Record<
					string,
					AgentToolDefinition['parameters']['properties'][string]
				>,
				required: tool.parameters?.required || []
			}
			// Note: execute function is NOT part of ToolDefinition - it's set via toolExecutor callback
		}));
	}
...
${activeModules.length > 0 ? activeModules.map((m) => `- ${m.name}: ${m.description || 'No description'}`).join('\n') : '- No modules active'}
...
After executing ANY tool, you MUST follow these steps:

1. **Process the Result**: Take the raw tool output and understand what it means.
2. **Summarize for the User**: Present the results in a clear, concise way:
   - **For lists** (tasks, goals, items, etc.): Use markdown tables with relevant columns
   - **For single values**: State them clearly in sentence form
   - **For empty results**: "You have no [items] matching your criteria" or similar
3. **Continue Don't Stop**: The tool execution is NOT the end of your response. Your response to the USER should include and explain the tool results.
4. **Be Actionable**: If results suggest a next step, mention it.

**Example flow:**
- User: "what tasks do i have?"
- Model calls get_tasks tool
- Tool returns: `[
			({ title: 'Review PR', status: 'todo' }, { title: 'Write docs', status: 'in_progress' })
		]`
- Model response: "You have **2 tasks**:
...
Would you like me to prioritize these or add any details?"
...
ASSISTANT:
<thought>
The user wants to see their tasks. I should call get_tasks to fetch the task list.
</thought>
*Calls get_tasks*
Tool returned: `[
			({ id: '1', title: 'Review PR', status: 'todo', priority: 'high' },
			{ id: '2', title: 'Write documentation', status: 'in_progress', priority: 'medium' })
		]`
**You have 2 tasks:**
...
Would you like me to prioritize these, add details, or create any new tasks?
</example>
