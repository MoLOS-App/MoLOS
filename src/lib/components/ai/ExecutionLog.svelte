			...
			{#each progress.executionLog as entry (entry.id)}
				<div>
					...
					{#if isExpanded(entry.id) && hasDetails(entry)}
						<div class="ml-6 mt-1 space-y-1 border-l-2 border-border/40 pl-3 text-[11px]">
							{#if entry.toolName}
							...
							{#if entry.parameters && Object.keys(entry.parameters).length > 0}
								<div class="space-y-1">
									<div class="font-medium text-muted-foreground">Parameters:</div>
									<div class="rounded border border-border/50 bg-background/50 p-2">
										<pre class="whitespace-pre-wrap font-mono text-[10px] text-foreground overflow-x-auto">{JSON.stringify(entry.parameters, null, 2)}</pre>
									</div>
								</div>
							{/if}
							{#if entry.result !== undefined}
								<div class="space-y-1">
									<div class="font-medium text-muted-foreground">Result:</div>
									<div class="rounded border border-border/50 bg-background/50 p-2">
										<pre class="whitespace-pre-wrap font-mono text-[10px] text-foreground overflow-x-auto">{formatValue(entry.result)}</pre>
									</div>
								</div>
							{/if}
							{#if entry.errorDetail}
								<div class="space-y-1">
									...
									<div class="rounded border border-destructive/50 bg-destructive/5 p-2">
										<pre class="whitespace-pre-wrap font-mono text-[10px] text-destructive overflow-x-auto">{entry.errorDetail}</pre>
									</div>
								</div>
							{/if}
						</div>
