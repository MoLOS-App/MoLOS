...
const LEGACY_CONFIG_DIR = path.join(process.cwd(), 'src/lib/config/modules');
const LEGACY_MODULES_DIR = path.join(process.cwd(), 'src/lib/modules');
...
export function cleanupModuleArtifacts(moduleId: string): void {
	...
	const paths = [
		...((Object.values(destinations).filter(Boolean) as string[]) || []),
		path.join(LEGACY_CONFIG_DIR, moduleId),
		path.join(LEGACY_MODULES_DIR, moduleId)
	];
