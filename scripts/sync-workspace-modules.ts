...
function createWorkspaceSymlinks(): void {
	const nodeModulesMolos = resolve(ROOT_DIR, 'node_modules/@molos');

	if (!existsSync(nodeModulesMolos)) {
		mkdirSync(nodeModulesMolos, { recursive: true });
	}
	...
	if (existsSync(packagesDir)) {
		...
		for (const entry of entries) {
			...
			try {
				...
				const symlinkPath = resolve(nodeModulesMolos, packageJson.name.replace('@molos/', ''));
				const targetPath = resolve(packagesDir, entry.name);

				if (existsSync(symlinkPath)) {
					unlinkSync(symlinkPath);
				}
				symlinkSync(targetPath, symlinkPath, 'dir');
				console.log(`[SyncDeps] Created symlink: ${packageJson.name} -> ${targetPath}`);
			} catch (e) {
				console.warn(`[SyncDeps] Warning: Could not process ${entry.name}`);
			}
		}
	}
}
