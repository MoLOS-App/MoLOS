...
-- Create migrations using: bun run db:migration:create --name initial --module ${moduleName}
...
\`\`\`bash
bun run module:sync    # Sync module routes
...
\`\`\`bash
bun run db:migration:create --name add_table --module ${moduleName}  # Create migration
...
`;
		...
		console.log('\nNext steps:');
		console.log('  1. Edit src/config.ts to customize navigation');
		console.log('  2. Run `bun run module:sync` from root to link module');
