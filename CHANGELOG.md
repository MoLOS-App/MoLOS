...
- **AI SDK Tool Format Compatibility**:
  - Fixed `MissingToolResultsError` by supporting both `tool-call` (AI SDK) and `tool_call` (legacy) formats in hasToolCalls/getToolCallIds
  - Fixed `ZodError: expected object, received string` by formatting `tool-result` output as `{ type: 'text'|'json', value: any }`
  - Fixed history parsing to convert JSON strings back to arrays for assistant messages
  - Enhanced tool progress display in UI with Wrench icon and "Running: {toolName}" status
  - See [AI SDK Tool Format Fix](./documentation/releases/ai-sdk-tool-format-fix.md) for details
