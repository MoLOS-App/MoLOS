<script lang="ts">
	...
	import MarkdownDisplayer from '$lib/components/shared/markdown/markdown-displayer.svelte';
...
{#if message.role === 'user' || processed.content.trim() !== '' || processed.thought || processed.plan || processed.attachments || processed.parts}
	...
	>
		...
		<div
			class="bubble-container relative min-w-0 rounded-2xl py-3 text-[14px] leading-relaxed transition-all duration-200 {message.role ===
			'user'
				? 'user-bubble max-w-[92%] bg-primary px-4 text-primary-foreground'
				: 'assistant-bubble max-w-full bg-muted/20 text-foreground'}"
		>
			...
			{:else}
				...
				{:else if processed.content.trim() !== ''}
					<div class="prose-sm prose prose-custom dark:prose-invert max-w-none">
						<MarkdownDisplayer source={processed.content} {renderers} />
					</div>
				{/if}
