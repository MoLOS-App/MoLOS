<script lang="ts">
	...
	async function loadMessages(sessionId: string, updateUrl = true) {
		...
		if (res.ok) {
			...
			if (updateUrl) {
				goto(`${$page.url.pathname}?session=${sessionId}`, { replaceState: true });
			}
			...
			for (const part of parts) {
				...
				switch (data.type) {
					...
					case 'tool_complete':
						...
						console.log('[Tool Complete]', data.toolName, data.result);
						// Store result preview for display in progress log
						if (data.result !== undefined && data.result !== null) {
							...
							addProgressLine(
								`📥 ${data.toolName}: ${resultPreview}${
									resultPreview && resultPreview.length >= 150 ? '...' : ''
								}`
							);
						}
						break;
	...
	function handleProgressEvent(event: any) {
		...
		switch (eventType) {
			...
			case 'step_complete':
				...
				// Log individual tool results if available
				if (eventData.toolCalls?.length > 0) {
					for (const tc of eventData.toolCalls) {
						...
						if (tc.result !== undefined && tc.result !== null) {
							...
							addProgressLine(
								`${toolLabel}: ${resultPreview}${resultPreview.length >= 100 ? '...' : ''}`
							);
						} else {
							addProgressLine(toolLabel);
						}
					}
				}
				break;
			...
			case 'tool_complete':
				// Tool call completed - display result preview
				console.log('[Tool Complete]', eventData.toolName, eventData.result);
				// Store result in currentAction for ProgressDisplay
				currentProgress.currentAction = {
					type: 'step_complete',
					message: `✓ ${eventData.toolName} completed`,
					timestamp: now,
					toolName: eventData.toolName,
					result: eventData.result
				};
				if (eventData.result !== undefined && eventData.result !== null) {
					// Format result preview (truncate to 300 chars for display)
					const resultStr =
					...
					addProgressLine(`📥 Result: ${resultPreview}`);
				}
				break;
...
<div class="flex h-full w-full flex-col overflow-hidden">
	<div class="flex h-full w-full flex-col overflow-hidden md:pr-4" in:fade={{ duration: 500 }}>
		<div
			class="flex h-full overflow-hidden md:min-h-[90svh] md:rounded-2xl md:border md:border-border/70"
		>
			...
			>
				...
				<div
					class="flex-1 overflow-x-hidden overflow-y-auto scroll-smooth px-4 py-6 md:px-6 md:py-8
"
