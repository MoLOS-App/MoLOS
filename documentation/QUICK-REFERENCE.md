...
| MissingToolResultsError (tool results dropped)             | Ensure agent code checks for both `tool-call` and `tool_call` formats (see releases/ai-sdk-tool-format-fix.md) |
| ZodError: expected object, received string (output field)  | Format `tool-result` output as `{ type: 'text'                                                                 | 'json', value: any }`, not raw string |
